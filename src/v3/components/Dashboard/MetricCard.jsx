...
const MetricCard = ({ title, value, subtext, icon: Icon, trend, accent = 'sage', onClick, progress }) => {
  const accentColors = {
  ...
  const pct = progress
    ? Math.min(100, progress.max > 0 ? (progress.current / progress.max) * 100 : 0)
    : null;

  return (
    <div
      onClick={onClick}
      className={`bg-white rounded-3xl p-8 border border-[#E8E4E1] shadow-sm transition-all group relative overflow-hidden
        ${onClick ? 'cursor-pointer hover:shadow-lg hover:scale-[1.02] active:scale-[0.99]' : 'hover:shadow-md'}`}
    >
        ...
        <div className="mt-4">
          <div className="h-1.5 bg-[#F2EFE9] rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${pct}%`, background: progressColor[accent] || '#5F6F65' }}
            />
          </div>
          {progress.label && (
            <p className="mt-1.5 text-[10px] font-bold text-[#9C8A7A]">{progress.label}</p>
          )}
        </div>
      )}

      {subtext && (
        <p className="mt-4 text-xs text-[#9C8A7A] leading-relaxed">{subtext}</p>
      )}
